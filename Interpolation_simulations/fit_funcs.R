...
if(!require("PolynomF")){install.packages("PolynomF")}else{library(PolynomF)}
...
lagrange_interp <- function(x, y, x_new) {
  ...
  for (i in seq_along(x_new)) {
    ...
    for (j in 1:n) {
      ...
      for (k in 1:n) {
        if (k != j) {
          product <- product * (x_new[i] - x[k]) / (x[j] - x[k])
        }
      }
      sum <- sum + product
    }
...
backfit=function(y,x,xgrid,d,k,mh,pi_init,sigma2_init,bw,backfit=TRUE,inter.method=1){
  n=length(y)
  ...
  if(backfit){
    ...
    while(diff>1e-10){
      ...
      if(inter.method==1){
        ...
        mu=sapply(1:k,function(j) splinefun(xgrid,mu1[,j])(x))
      }
...
Kernel_Mix_EM_loc=function(x,y,k,bw,d,xgrid,init.model,inter.method=1){
  n=length(y)
  ngrid=length(xgrid)
  ##Initial state
  ...
  while(diff>1e-10){
    ...
    if(inter.method==1){
      ...
      for(j in 1:k){
        ...
        pi1=cbind(pi1,local.polynomial.smoother(x,rep(1,n),xgrid,bw,0,W)[,2])
        mh=local.polynomial.smoother(x,y,xgrid,bw,d,W)[,2]
        mu1=cbind(mu1,splinefun(xgrid,mh)(x))
        sigma21=cbind(sigma21,local.polynomial.smoother(x,(y-mu1[,j])^2,xgrid,bw,0,W)[,2])
      }
      pi1=sapply(1:k,function(j) splinefun(xgrid,pi1[,j])(x))
      sigma21=sapply(1:k,function(j) splinefun(xgrid,sigma21[,j])(x))
    }
    ##Evaluating convergence
    LogLik1=sum(log(rowSums(sapply(1:k,function(j) pi1[,j]*dnorm(y-mu1[,j],0,sqrt(sigma21)[,j])))))
    diff=abs(LogLik0-LogLik1)
  ...
  out=list(resp=gn,mix.prop=pi1,mix.mu=mu1,mix.sigma2=sigma21,df=df,LLiter=tol,LL=LL1,R2=R2,BIC=BIC,est.dist=c.dist,est.cdf=est.cdf)
  return(out)
}
...
Kernel_Mix_EM.Naive=function(x,y,k,bw,d,xgrid,init.model){
  ...
  df=sum(sapply(1:k,function(j){w=gn[,j]*Kh;S=polynomial.smoother.matrix(x,x,d,w);sum(diag(S))}))
  BIC=BIC(x,bw,k,LL1,df)
  out=list(resp=gn,mix.prop=pi1,mix.mu=mu1,mix.sigma2=sigma21,df=df,LL=LL1,R2=R2,BIC=BIC,est.dist=c.dist,est.cdf=est.cdf,sw=res$switch,LLiter=tol)
  return(out)
}

###Modified-Naive EM algorithm
Kernel_Mix_EM.Naive2=function(x,y,k,bw,d,xgrid,init.model,iter=FALSE){
  n=length(y)
  ngrid=length(xgrid)
  ##Initial state
  pi0=init.model$mix.prop
  ...
  while(diff>1e-10){
    ##local E-step
    g=lapply(1:ngrid,function(i){g=sapply(1:k,function(j) pi0[j]*dnorm(y-mu0[i,j],0,sqrt(sigma20[j]))+1e-100);gn=g/rowSums(g)})
    ##local M-step
    mu1=t(sapply(1:ngrid,function(t){
      sapply(1:k,function(j){
        w=g[[t]][,j]
    ...
    mu=sapply(1:k,function(j) approx(xgrid,mu1[,j],x,rule=2)$y)
    ##Evaluating convergence
    LogLik1=sum(log(rowSums(sapply(1:k,function(j) pi0[j]*dnorm(y-mu[,j],0,sqrt(sigma20)[j])))))
    diff=abs(LogLik0-LogLik1)
    LogLik0=LogLik1
    mu0=mu1
    count=count+1
  ...
  mu=sapply(1:k,function(j) approx(xgrid,mu1[,j],x,rule=2)$y)
  mu1=mu
  if(iter){out=backfit_fullyIter(y,x,xgrid,d,k,mu1,pi0,sigma20,bw)}
  else{out=backfit(y,x,xgrid,d,k,mu1,pi0,sigma20,bw)}
  mu1=out$mu;pi1=out$pi1;sigma21=out$sigma21;
  res=Dist(mu1);mu1=mu1[,res$id];pi1=pi0[res$id];sigma21=sigma20[res$id]
  g=sapply(1:k,function(j) pi1[j]*dnorm(y-mu1[,j],0,sqrt(sigma21)[j]));gn=g/rowSums(g)
  R2=Rsquared(y,mu1,gn)
  LL1=sum(log(rowSums(g)))
  c.dist=cond_dist(y,mu1,pi1,sigma21)
  ...
  df=sum(sapply(1:k,function(j){w=gn[,j]*Kh;S=polynomial.smoother.matrix(x,x,d,w);sum(diag(S))}))
  BIC=BIC(x,bw,k,LL1,df)
  out=list(resp=gn,mix.prop=pi1,mix.mu=mu1,mix.sigma2=sigma21,df=df,LL=LL1,R2=R2,BIC=BIC,est.dist=c.dist,est.cdf=est.cdf,sw=res$switch)
  return(out)
}

###Estimator = all the remaining grid points
Kernel_Mix_EM_MB2=function(x,y,k,bw,d,xgrid,init.model,lmd_0=1e-5){
  n=length(y)
  ngrid=length(xgrid)
  grid=1:ngrid
  xgrid0=xgrid
  ...
  while(diff>1e-10){
    ...
    if(inter.method==1){
      ...
      for(j in 1:k){
        mix.prop=cbind(mix.prop,lagrange_interp(xgrid,prop[,j],xout=x))
        mix.sigma2=cbind(mix.sigma2,lagrange_interp(xgrid,sigma2[,j],xout=x))
        mix.mu=cbind(mix.mu,lagrange_interp(xgrid,mu[,j],xout=x))
      }
    }
  ...
  LL1=sum(log(rowSums(g)))
  R2=Rsquared(y,mu2,gn)
  c.dist=cond_dist(y,mu2,pi1,sigma21)
  est.cdf=CDFmixture(x,y,pi1,sigma21,mu2)$Fy
  Kh=sapply(x,function(x0) Kern(x,x0,bw))
  df=sum(sapply(1:k,function(j){w=gn[,j]*Kh;S=polynomial.smoother.matrix(x,x,d,w);sum(diag(S))}))
  BIC=BIC(x,bw,k,LL1,df)
  out=list(resp=gn,mix.prop=pi1,mix.mu=mu2,mix.sigma2=sigma21,df=df,LL=LL1,R2=R2,BIC=BIC[2],est.dist=c.dist,est.cdf=est.cdf,sw=res$switch,LLiter=tol)
  return(out)
}

###Estimator = all the local models
Kernel_Mix_EM_MB3=function(x,y,k,bw,d,xgrid,init.model,lmd_0=NULL,inter.method=1){
  n=length(y)
  ngrid=length(xgrid)
  ##Initial state
  lmd0=rep(1/ngrid,ngrid)
  ...
  while(diff>1e-10){
    ...
    if(inter.method==1){
      ...
      for(j in 1:k){
        mix.prop=cbind(mix.prop,splinefun(xgrid,prop[,j])(x))
        mix.sigma2=cbind(mix.sigma2,splinefun(xgrid,sigma2[,j])(x))
        mix.mu=cbind(mix.mu,splinefun(xgrid,mu[,j])(x))
      }
    }
    print(mix.sigma2)
    LogLik1=sum(log(rowSums(sapply(1:k,function(j) mix.prop[,j]*dnorm(y-mix.mu[,j],0,sqrt(mix.sigma2)[,j])))))
